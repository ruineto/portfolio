...
{
    public partial class AddSumario : Form {

        string strConn = Form1.strConn;
        private Form1 ParentForm;
        private bool creating = true;
        public AddSumario(Form1 form)
        {
            InitializeComponent();
            ParentForm = form;
            fillComboBoxs();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection myConnection = new SqlConnection(strConn))
            {
                ...
                {
                    cmd.CommandText = "EXEC CriarSumario "+idTurma+", '"+disc+"', '"+textBoxSumario.Text.ToString()+"', '"+dateTimePicker.Value.Date+"';";
                    try
                    {
                        cmd.ExecuteNonQuery();
                    }

...
{
    public partial class RemoveEstudanteTurma : Form
    {
        string strConn = Form1.strConn;
        private Form1 ParentForm;
        private bool creating = true;
        public RemoveEstudanteTurma(Form1 form)
        {
            InitializeComponent();
            ParentForm = form;
            fillComboBoxs();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show(idTurma+" "+idAluno);
            ...
            using (SqlConnection myConnection = new SqlConnection(strConn))
            {
                ...
                {
                    cmd.CommandText = "EXEC ApagarEstudanteTurma " + idAluno + ", " + idTurma + ";";
                    try
                    {
                        cmd.ExecuteNonQuery();
                    }
        ...
        private void comboBoxTurmas_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (creating != true)
            {
                ...
                using (SqlConnection myConnection = new SqlConnection(strConn))
                {
                    string turma = ((KeyValuePair<Int32, string>)comboBoxTurmas.SelectedItem).Value;
                    string query = "select Nome, BI, Nif, DataNascimento as 'Data Nascimento', Email, Morada from GET_ALUNOS where Turma='" + turma + "';";
                    myConnection.Open();
                    using (SqlCommand cmd = new SqlCommand(query, myConnection))
                    {
